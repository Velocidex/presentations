...
func ParsePresentation() (*Presentation, error) {
	root := &Presentation{}
...
func buildPresentationTOC(presentation *Presentation) string {
...
`,
			...
			module_path, getPresentationTopics(module))
	}
...
func getPresentationSlides(topic *Topic, topic_link string) string {
	res := `
 <ul>
`
	last_title := ""
...
// group of slides.
func getPresentationTopics(module *Module) string {
	res := `
 <ul>
`
	for _, topic := range module.Topics {
...
`, topic.Link, topic.Name, getPresentationSlides(topic, topic.Link))
	}
	res += `
 </ul>
`
